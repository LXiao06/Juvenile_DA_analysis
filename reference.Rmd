...
| **Population Analysis** | `combine_animal_results()`, `analyze_td_metrics()`, `analyze_amplitude_metrics()`, `analyze_cH_metrics()`, `extract_photometry_data()`, `pool_photometry_data()`, `save_npy()` | Combine, pool, and analyze population-level patterns |
...
**`extract_photometry_data()`** - Extracts trial-aligned fiber photometry epoch data for specified developmental labels or conditions from an NPM object.

- **Purpose**: Extracts photometry segments for individual developmental stages
- **Key Parameters**: `npm`, `data_type`, `region`, `labels`, `ref_line`, `total_window`, `pre_motif_window`
- **Used in**: [Photometry Modeling](photometry_modeling.html)
- **Documentation**: Available in VNS man pages

**`pool_photometry_data()`** - Pools individual animal data matrices into a single multi-trial matrix, with optional trial-number balancing.

- **Purpose**: Combines multiple matrices and balances trial numbers across subjects
- **Key Parameters**: `data_list`, `balance`
- **Used in**: [Photometry Modeling](photometry_modeling.html)
- **Documentation**: Available in VNS man pages

**`save_npy()`** - Exports pooled R data arrays to NumPy `.npy` files for compatibility with Python-based pipelines.

- **Purpose**: Exports data to Python-compatible `.npy` files
- **Key Parameters**: `data`, `output_dir`, `python_path`, `file_prefix`
- **Used in**: [Photometry Modeling](photometry_modeling.html)
- **Documentation**: Available in VNS man pages

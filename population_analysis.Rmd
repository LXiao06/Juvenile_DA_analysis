...
The `population_data.rda` file contains the following pre-processed datasets:

**`temporal_shift_res`** - Population-level temporal dynamics dataset containing dopamine transient timing metrics across development for all animals. Used for analyzing temporal difference learning patterns including peak timing, backward shift rates, temporal entropy, and Gini coefficients.

**`amplitude_shift_res`** - Population-level amplitude analysis dataset containing peak amplitude measurements from early and late motif segments across development. Used for examining amplitude redistribution patterns characteristic of temporal difference learning.

**`amplitude_within_window_res`** - Population-level amplitude redistribution dataset for within-window sliding analysis. Contains amplitude measurements across different segment pairs within 6-day temporal windows for examining short-term developmental changes.

**`cH_res`** - Population-level conditional entropy dataset containing directional information flow measurements between early and late motif segments. Used for analyzing predictive coding development and information asymmetry across animals.

**`cH_within_window_res`** - Population-level information flow dataset for within-window sliding analysis. Contains conditional entropy measurements within 6-day temporal windows for examining short-term changes in predictive relationships.

These datasets represent the combined results from individual animal analyses (as demonstrated in the individual analysis tutorial) and are ready for population-level statistical modeling and visualization.

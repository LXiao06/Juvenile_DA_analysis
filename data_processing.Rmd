...
This document demonstrates the data processing pipeline for analyzing longitudinal fiber photometry dopamine data recorded from vocal basal ganglia in juvenile birds. The analysis uses the VNS and ASAP packages to process both fiber photometry signals and corresponding song recordings.
...
**Photometry Data Import and Quality Control**
...
npm <- npm |>
  ...
  process_photometry(
    by_epoch = TRUE, 
    epoch_window_quantile = 0.9)
```

**Synchronization and Transient Detection**
...
# Save individual npm object
saveRDS(npm, "./data/B633.rds")
```

...
def plot_trial_average_col(ax, data, model, total_window, motif_onset,
    ...
    ax.axvline(x=motif_onset, color="k", linestyle="--", label="Motif Onset")
    ax.set_xlabel("Time (s)")
...
datasets = [
    (early_dev, early_dev_model, JUV_TOTAL_WINDOW,   JUV_MOTIF_ONSET,   "#E07040"),
    (late_dev,  late_dev_model,  JUV_TOTAL_WINDOW,   JUV_MOTIF_ONSET,   "#4070E0"),
    (raw_adult, adult_model,     ADULT_TOTAL_WINDOW, ADULT_MOTIF_ONSET, "#40A060"),
]
...
for col, (data, model, tw, mo, col_color) in enumerate(datasets):
    ...
    # Row 0: Raw heatmap
    plot_heatmap_col(axes[0, col], data, vmin, vmax, tw,
                     title=col_titles[col])

    # Row 1: Model estimate  (model.predict() with its own normalisation)
    plot_model_estimate_col(axes[1, col], model, tw, data.shape[0])

    # Row 2: Shift-aligned raw
    plot_heatmap_col(axes[2, col], model.transform(data), vmin, vmax, tw)
...
fig.suptitle("Shift-Only Time Warping of Pooled dLight — Developmental Stages",
             fontsize=15, y=1.002)
...
```{python, plot_template_comparison, fig.width=9, fig.height=4, eval = EVAL}
common_n = 200
x = np.linspace(0, 1, common_n)

t_early_n = np.interp(x, np.linspace(0, 1, len(t_early)), t_early)
t_late_n  = np.interp(x, np.linspace(0, 1, len(t_late)),  t_late)
t_adult_n = np.interp(x, np.linspace(0, 1, len(t_adult)), t_adult)

fig, ax = plt.subplots(figsize=(9, 4))
ax.plot(x, t_early_n, label="Early Development", color="#E07040", lw=2)
ax.plot(x, t_late_n,  label="Late Development",  color="#4070E0", lw=2)
ax.plot(x, t_adult_n, label="Full Adult",        color="#40A060", lw=2, ls="--")
ax.axhline(0, color="gray", lw=0.8, ls="--")
ax.set_xlabel("Normalized time (0 = window start, 1 = window end)")
ax.set_ylabel("dF/F (template)")
ax.set_title("Warping Template Comparison Across Developmental Stages")
ax.legend()
fig.tight_layout()
